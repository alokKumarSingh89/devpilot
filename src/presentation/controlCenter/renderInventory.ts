...
export function renderInventory(state: InventoryState, canScan: boolean): string {
  ...
  const types = { SINGLE_PACKAGE: 'Single package', MONOREPO: 'Monorepo', MULTI_PROJECT: 'Multiple projects', UNKNOWN: 'Unknown' };
  return `<div class="prd-input"><h4>Codebase intelligence</h4><p role="status">${labels[state.status]}</p>
    ...
    ${inventory ? `<dl class="project-details"><dt>Repository</dt><dd>${types[inventory.repository.type]}</dd><dt>Languages</dt><dd>${escapeHtml(inventory.languages.map((item) => item.name).join(', ') || 'Unknown')}</dd><dt>Frameworks</dt><dd>${escapeHtml(inventory.frameworks.map((item) => item.name).join(', ') || 'None detected')}</dd><dt>Package managers</dt><dd>${escapeHtml(inventory.tooling.packageManagers.join(', ') || 'None detected')}${inventory.tooling.packageManager.conflict ? ' · Conflicting evidence' : ''}</dd><dt>Database / ORM signals</dt><dd>${escapeHtml(inventory.technologies.filter((item) => item.kind === 'DATABASE' || item.kind === 'ORM').map((item) => item.name).join(', ') || 'None detected')}</dd><dt>Packages</dt><dd>${inventory.packages.length}</dd><dt>Tests</dt><dd>${inventory.testing.testFileCount} files</dd><dt>Git</dt><dd>${inventory.git.available ? `${escapeHtml(inventory.git.branch ?? 'Detached / unborn HEAD')} · ${inventory.git.dirty ? 'Modified' : 'Clean'}` : 'Unavailable'}</dd></dl>
