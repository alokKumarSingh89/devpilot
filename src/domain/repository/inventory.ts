import { LANGUAGE_CATALOG, FRAMEWORK_CATALOG, PACKAGE_MANAGER_CATALOG, TEST_CATALOG, BUILD_CATALOG, PROJECT_KINDS, type DetectedTechnology } from './technology';
...
export const MANIFEST_TYPES = ['NPM_PACKAGE', 'LOCKFILE', 'WORKSPACE_CONFIG', 'TYPESCRIPT_CONFIG', 'PYTHON_PROJECT', 'PYTHON_REQUIREMENTS', 'MAVEN', 'GRADLE', 'GO_MODULE', 'RUST_PACKAGE', 'DOCKER', 'CI_WORKFLOW', 'TOOL_CONFIG', 'PYTHON_SETUP', 'PIPFILE', 'DOTNET_PROJECT', 'DOTNET_SOLUTION', 'RUBY_GEMFILE', 'PHP_COMPOSER', 'SWIFT_PACKAGE', 'DART_PACKAGE', 'PRISMA_SCHEMA'] as const;
export type ManifestType = typeof MANIFEST_TYPES[number];
export const LANGUAGES = LANGUAGE_CATALOG.map((item) => item.name);
export const FRAMEWORKS = FRAMEWORK_CATALOG.map((item) => item.name);
export const TEST_FRAMEWORKS = TEST_CATALOG;
export const BUILD_TOOLS = BUILD_CATALOG;
export const PACKAGE_MANAGERS = [...PACKAGE_MANAGER_CATALOG.map((item) => item.name), 'UNKNOWN'] as const;
...
export interface Inventory {
  ...
  readonly repository: { readonly workspaceName: string; readonly type: 'SINGLE_PACKAGE' | 'MONOREPO' | 'MULTI_PROJECT' | 'UNKNOWN'; readonly evidence: readonly Evidence[] };
  readonly languages: readonly { readonly id: string; readonly name: typeof LANGUAGES[number]; readonly fileCount: number }[];
  readonly manifests: readonly { readonly relativePath: string; readonly type: ManifestType; readonly contentHash: string | null }[];
  readonly packages: readonly { readonly id: string; readonly name: string; readonly relativePath: string; readonly kind: typeof PROJECT_KINDS[number]; readonly manifestPath: string }[];
  readonly frameworks: readonly { readonly id: string; readonly category: typeof FRAMEWORK_CATALOG[number]['category']; readonly name: typeof FRAMEWORKS[number]; readonly evidence: readonly Evidence[] }[];
  readonly testing: { readonly frameworks: readonly typeof TEST_FRAMEWORKS[number][]; readonly testFileCount: number };
  readonly tooling: { readonly packageManagers: readonly typeof PACKAGE_MANAGERS[number][]; readonly packageManager: { readonly name: typeof PACKAGE_MANAGERS[number]; readonly conflict: boolean; readonly evidence: readonly Evidence[] }; readonly buildTools: readonly typeof BUILD_TOOLS[number][] };
