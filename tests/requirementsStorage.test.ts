...
import { VscodeRequirementsStorage, REQUIREMENTS_PATH } from '../src/infrastructure/requirements/VscodeRequirementsStorage';
import { serializeRequirementsYaml } from '../src/infrastructure/requirements/requirementsYaml';
...
const destination = `${workspace.key}/${REQUIREMENTS_PATH}`;
const encode = (text: string) => new TextEncoder().encode(text);
const logger = vi.fn(); const storage = new VscodeRequirementsStorage(logger);
const commit = vi.fn(async () => undefined);
function saved() { return new TextDecoder().decode(entries.get(destination)?.bytes); }
function seed() { entries.set(destination, { type: 1, bytes: encode(serializeRequirementsYaml(requirementsArtifact())) }); }
...
describe('requirements artifact persistence', () => {
  it('publishes a complete validated artifact through temporary-file rename then restores it', async () => {
    ...
    await storage.write(workspace, requirementsArtifact(), commit);
    expect(await storage.read(workspace)).toEqual(requirementsArtifact());
    expect(host.fs.rename).toHaveBeenCalledWith(expect.objectContaining({ key: expect.stringMatching(/\.requirements-.*\.tmp$/) }), { key: destination }, { overwrite: false });
  ...
  it('does not touch the filesystem for invalid output', async () => {
    ...
    await expect(storage.write(workspace, { ...requirementsArtifact(), schemaVersion: 2 } as unknown as ReturnType<typeof requirementsArtifact>, commit)).rejects.toMatchObject({ code: 'INVALID_ARTIFACT' });
  ...
  it('replaces a previous artifact only after successful validation and write', async () => {
    seed(); const next = { ...requirementsArtifact(), product: { name: 'Revised', summary: 'A reviewed task tracker.' } };
  ...
  it.each(['write', 'rename', 'cancel', 'source'])('preserves valid requirements after %s failure and removes temporary output', async (stage) => {
    ...
    if (stage === 'cancel') commit.mockResolvedValueOnce(undefined).mockRejectedValueOnce(new ModelFailure('CANCELLED'));
    if (stage === 'source') commit.mockResolvedValueOnce(undefined).mockRejectedValueOnce(new AnalysisFailure('CHANGED'));
    await expect(storage.write(workspace, requirementsArtifact(), commit)).rejects.toBeInstanceOf(Error);
  ...
  it('detects competing artifact edits during write and leaves them intact', async () => {
    ...
    await expect(storage.write(workspace, requirementsArtifact(), commit)).rejects.toMatchObject({ code: 'CONFLICT' });
    expect(saved()).toBe('another writer'); expect(host.fs.rename).not.toHaveBeenCalled();
  });
  it.each(['.devpilot', '.devpilot/product', REQUIREMENTS_PATH])('rejects symlinked metadata path %s', async (path) => {
    entries.set(`${workspace.key}/${path}`, { type: 65, bytes: encode('data') });
    await expect(storage.read(workspace)).rejects.toMatchObject({ code: 'INVALID_ARTIFACT' });
    await expect(storage.write(workspace, requirementsArtifact(), commit)).rejects.toMatchObject({ code: 'INVALID_ARTIFACT' });
  ...
  it('rejects corrupt UTF-8 and oversized persisted artifacts', async () => {
    ...
    await expect(storage.read(workspace)).rejects.toMatchObject({ code: 'INVALID_ARTIFACT' });
    entries.set(destination, { type: 1, bytes: new Uint8Array(1048577) });
    host.fs.readFile.mockClear();
    await expect(storage.read(workspace)).rejects.toMatchObject({ code: 'INVALID_ARTIFACT' }); expect(host.fs.readFile).not.toHaveBeenCalled();
  });
